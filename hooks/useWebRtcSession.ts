...
} from "../components/webrtcEventHandlers";
import { fetchOpenAiEphemeralKey } from "../components/webrtcUtils";
...
interface UseWebRtcSessionArgs {
...
export const useWebRtcSession = ({
  ...
  const addLog = useCallback(
    (message: string) => {
      logger.log(
        `[CW_LOG:${activeSessionId?.substring(0, 6)}]: ${message}`,
      );
    },
    [activeSessionId],
  );
